...
is_booking:true если клиент явно хочет записаться ("запишите", "хочу записаться", "можно на...") ИЛИ есть конкретное имя + время.
"""
...
- Приблизительное время (завтра, утром, в пятницу) — valid:true
- Вне рабочего времени (воскресенье 22:00) — valid:false с объяснением
- Бессмысленное — valid:false, issue:"Укажите дату и время — например, завтра утром или в пятницу после 18:00"
"""
...
def self_ping():
    ...
    while True:
        ...
        try:
            urllib.request.urlopen(RENDER_URL + "/ping", timeout=10)
...
def handle_message(message):
    ...
    # ── Шаг: выбор услуги текстом ───────────────────────────────
        ...
        bot.send_message(cid, "Пожалуйста, выберите услугу из списка 👇", reply_markup=service_menu())
