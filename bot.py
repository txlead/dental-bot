...
def handle_message(message):
    ...
    if state == "waiting_name":
        if len(text) > 50 or any(c.isdigit() for c in text):
            ...
            return
        booking_data.setdefault(cid, {})["name"] = text
        user_state[cid] = "waiting_phone"
        bot.send_message(
            cid,
    ...
    # ── Шаг: время ──────────────────────────────────────────────
        ...
        # Обычная проверка по подстроке
        is_question = any(sig in text_lower for sig in question_signals)
        # Отдельные слова — только как целые слова чтобы не было ложных срабатываний
        import re as _re
        if not is_question:
            is_question = bool(_re.search(r'\bлет\b|\bгод\b', text_lower))
