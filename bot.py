...
BOT_TOKEN  = os.environ.get("BOT_TOKEN",  "8603016078:AAHFR1NmT3RPlQn2lWGVI01M0xfZTQ3v0gM")
OPENAI_KEY = os.environ.get("OPENAI_KEY", "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")
# ВАШ Telegram user_id — чтобы получать уведомления о заявках и вопросах к админу
...
user_state = {}
booking_data = {}  # временное хранение данных записи
...
def handle_callback(call):
    ...
    # Сброс состояния записи при навигации
    ...
    if call.data == "prices":
        ...
        bot.send_message(cid,
            "🦷 *Профчистка зубов* — от 3 500 руб\n"
            "Включает ультразвук + полировку\n\n"
            "Записаться на чистку?",
            parse_mode="Markdown", reply_markup=after_price_menu())
...
def handle_message(message):
    ...
    if state == "waiting_name":
        ...
        bot.send_message(cid,
            ...
            "(например: чистка, лечение кариеса, имплант, отбеливание, консультация)")
    ...
    # ── Свободный вопрос через кнопку ───────────────────────────────────────
    elif state == "waiting_question":
        user_state.pop(cid, None)
        _ai_respond(cid, text)

    # ── Любое другое сообщение — AI отвечает ────────────────────────────────
    else:
        _ai_respond(cid, text)

def _ai_respond(cid, text):
    """Отправить вопрос в GPT и ответить клиенту."""
    try:
        ...
        # Если AI говорит что передаёт вопрос — уведомляем администратора
        if "администратор" in answer.lower() or "передал" in answer.lower():
            notify_admin(
                f"❓ ВОПРОС К АДМИНИСТРАТОРУ\n\n"
                f"💬 Клиент спросил: {text}\n"
                f"🤖 Ответ бота: {answer}\n"
                f"📌 Chat ID: {cid}"
            )
