...
Правила поведения:
1. Если клиент хочет записаться — скажи что оформим запись через кнопку "Записаться" и предложи нажать её.
2. Если вопрос про что-то чего нет в прайсе (брекеты, ортодонтия, пластины и т.д.) — скажи тепло: "Этот вопрос я передал администратору — вам ответят в течение 2 часов 😊"
...
6. На грубость отвечай спокойно и с лёгким юмором, не обижайся.
7. Всегда заканчивай ответ предложением действия — записаться, посмотреть цены или задать вопрос.
"""
...
"хочу брата на чистку" → {"is_booking":false,"name":null,"time":null,"time_valid":true,"time_issue":null}

Важно: is_booking:true ТОЛЬКО если есть конкретное имя собственное (Саша, Захар), НЕ просто "я хочу".
"""

TIME_CHECK_PROMPT = """
Проверь является ли текст реальным временем для записи к врачу.
Режим работы клиники: пн-пт 9:00-21:00, сб-вс 10:00-18:00.
Ответь ТОЛЬКО валидным JSON без markdown:
{"valid": true/false, "issue": "описание проблемы или null"}

Правила:
- Приблизительное время (завтра, утром, на следующей неделе, в пятницу) — valid:true
- Конкретное время вне работы (воскресенье 22:00, понедельник 8:00) — valid:false с объяснением
- Бессмысленные ответы ("никогда", "не знаю", случайные слова, оскорбления) — valid:false, issue:"Пожалуйста, укажите удобную дату и время — например, завтра утром или в пятницу после 18:00"
"""
...
def service_menu():
    ...
    m.row(InlineKeyboardButton("🔩 Имплант", callback_data="select_implant"),
          InlineKeyboardButton("✨ Отбелить", callback_data="select_white"))
...
def service_accusative(service):
    mapping = {
...
def handle_callback(call):
    ...
    if data == "back_to_start":
        ...
        bot.send_message(cid,
            f"Отлично! Записываем вас на *{service_accusative(service)}* ✅\n\n"
...
def self_ping():
    """Пингуем себя каждые 4 минуты чтобы не засыпать на Render."""
    import time
    import urllib.request
    url = "https://dental-bot-p20r.onrender.com"
    while True:
...
if __name__ == "__main__":
    ...
    ping_thread.start()
    print("Bot started ✅")
    bot.polling(none_stop=True, interval=1)    "select_implant": "Имплантация",
    "select_white":   "Отбеливание",
    "select_consult": "Консультация (бесплатно)",
    "price_clean":    "Профчистка зубов",
    "price_treat":    "Лечение кариеса",
...
if __name__ == "__main__":
    ...
    t.start()
    print("Bot started ✅")
    bot.polling(none_stop=True, interval=1)        f"Администратор свяжется с вами в ближайшее время для подтверждения.",
        parse_mode="Markdown", reply_markup=only_back())
    notify_admin(f"🔔 НОВАЯ ЗАЯВКА\n\n👤 {name}\n🦷 {service}\n🕐 {time_str}\n💬 Chat ID: {cid}")
